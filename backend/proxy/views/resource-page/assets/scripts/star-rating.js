...
document.getElementById('rate-button').onclick=function(){
    ...
    console.log(window.location.href+"/rating");
    xhr.open("POST", window.location.href + "/rating");
    xhr.addEventListener("load", function loadCallback(){
        let response = JSON.parse(xhr.response);
        //console.log(response);
        if (response.status == "error") {
            document.getElementById('rate-button').innerText="Deja notata!";
        } else {
           document.getElementById('rate-button').innerText="Resursa notata cu succes!";
        }
    });
