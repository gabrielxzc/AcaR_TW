...
exports.model = (materie, page, callback) => {
    oracledb.getConnection(databaseConfig, (error, connection) => {
        ...
        connection.execute(
            'SELECT titlu, autor, anul_publicarii, link, imagine FROM (SELECT rownum as page, titlu, autor, anul_publicarii, link, imagine FROM carti) WHERE page BETWEEN (:page - 1) * 5 + 1 AND :page * 5', {
                page: page
            },
            (error, result) => {
                if (error) {
